...
class SpeedVsCurvatureExperiment(object):
    ...
    def __init__(self, curvature_radius, robot_velocity):
        # class variables
    ...
    def odom_callback(self, odom):
        ...
        if not self.velocity_reached:
            ...
            if odom.twist.twist.linear.x >= self.robot_vel:
                ...
                curv_time = (pi*self.curv_R) / self.robot_vel # half circle
        ...
        if self.simend_time != -1 and self.time < self.simend_time + 0.5:
    ...
    def publish_paths(self):
        ...
        for i in range(1000):
            pose_stamped = PoseStamped()
            x = self.cx + self.curv_R*cos(theta0 - pi/1000.0*i) # direction
            y = self.cy + self.curv_R*sin(theta0 - pi/1000.0*i) # direction
    ...
    def run(self):
        if not self.paths_published and self.simend_time != -1 and self.time > self.simend_time + 0.5:
            self.angle = 0
            self.robot_vel = 0
...
if __name__ == '__main__':
    ...
    experiment = SpeedVsCurvatureExperiment(10.0, 2.0)
